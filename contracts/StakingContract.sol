...
contract ERC721Staking is ReentrancyGuard,ChainlinkClient, ConfirmedOwner {
    ...
    event RequestLevel(bytes32 indexed requestId, uint256 level);
    // Interfaces for ERC20 and ERC721
    IERC20 public immutable rewardsToken;
    ERC721 public immutable nftCollection;

    // Constructor function to set the rewards token and the NFT collection addresses
    constructor(ERC721 _nftCollection, IERC20 _rewardsToken)ConfirmedOwner(msg.sender) {
    ...
    function requestLevelData(uint256 _tokenId) public returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);

        // Set the URL to perform the GET request on
        req.add('get', getUrl(nftCollection.tokenURI(_tokenId)));
        req.add('path', 'attributes,level'); // Chainlink nodes 1.0.0 and later support this format
        int256 timesAmount = 10;
        req.addInt('times', timesAmount);
        return sendChainlinkRequest(req, fee);
    }
    function fulfill(bytes32 _requestId, uint256 _level) public recordChainlinkFulfillment(_requestId) {
        emit RequestLevel(_requestId, _level);
        level = (level + _level)/levelCount;
    ...
    function getUrl(string memory _tokenId)
        public
        pure
        returns (string memory url)
    {
        bytes memory str = bytes(_tokenId);
        uint256 length_str = utfStringLength(_tokenId);
        bytes memory result = new bytes(length_str - 7);

        for(uint256 i = 7;i < length_str; i++){
            result[i - 7] = str[i];
        }
        string memory ipfsHttpUrl = "https://ipfs.io/ipfs/";
        url = string(abi.encodePacked(ipfsHttpUrl,string(result)));
    }
